...
% nln = number of local nodes (degrees of freedom) per element.
nln = 2;  % A 1d interval has 2 nodes and 2 linear basis functions

% ?????????? WHAT IS THIS BELOW
%bound_pts = ones(length(Region.coord),1); %(1 1 ... 1) long as number of points used to discretize

% Find the indexes in which the boundary points are:
