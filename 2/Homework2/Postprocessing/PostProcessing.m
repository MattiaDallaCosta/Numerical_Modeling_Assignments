function [solutions] = PostProcessing(Data, femregion, eta_h_T, q_h_T)
%% PostProcessing(Data,femregion,eta_h_T,q_h_T)
%==========================================================================
% Post processing at final time T for eta and q.
%==========================================================================

fprintf('\n Plot the solution ... \n');

x = femregion.dof(:,1);
x_ex = linspace(Data.domain(1),Data.domain(2),1000);

% Handle periodic reduction mismatch
if length(eta_h_T) ~= length(x)
    x = x(1:length(eta_h_T));
end

% Exact solutions at final time if available
eta_ex_T = [];
q_ex_T   = [];
if isfield(Data,'etaex') && isa(Data.etaex,'function_handle')
    eta_ex_T = Data.etaex(x_ex, Data.PicTime);
end

if isfield(Data,'qex') && isa(Data.qex,'function_handle')
    q_ex_T = Data.qex(x_ex, Data.PicTime);
end

fs         = 35;
lw         = 1.9;
fnt        = 'Times New Roman';
interp_tex = 'latex';

clr = [ ...
    0.00  0.45  0.74;   %  1  blue
    0.85  0.33  0.10;   %  2  orange
    0.47  0.67  0.19;   %  3  green
    0.49  0.18  0.56;   %  4  purple
    0.93  0.69  0.13;   %  5  yellow
    0.30  0.75  0.93;   %  6  cyan
    0.64  0.08  0.18;   %  7  dark red
    0.25  0.25  0.25;   %  8  dark gray
    0.00  0.60  0.50;   %  9  teal
    0.75  0.00  0.75;   % 10  magenta
    0.60  0.60  0.60];  % 11  light gray

if Data.visual_graph

    fig1 = figure('Units','centimeters','Position',[2 2 30 24], ...
                  'Color','w','PaperPositionMode','auto');

    % ---------- SUBPLOT 1: eta ----------
    subplot(2,1,1);
    hold on; box on; grid on;
    set(gca, 'GridLineStyle','-', 'GridAlpha',0.15);

    plot(x, full(eta_h_T), '-', 'LineWidth',lw, 'Color',clr(1,:));
    if ~isempty(eta_ex_T)
        plot(x_ex, eta_ex_T, '--', 'LineWidth',lw, 'Color',clr(2,:));
    end

    title(sprintf('$\\eta$ at time %.1f\\,s with $\\Delta t=%.2f$\\,ms, $h=%.2f$, $p=%g$.', ...
          Data.PicTime, Data.dt*1000, 1/femregion.ne, Data.p), ...
          'Interpreter',interp_tex, 'FontName',fnt, 'FontSize',fs);
    xlabel('$x$',    'Interpreter',interp_tex, 'FontName',fnt, 'FontSize',fs);
    ylabel('$\eta$', 'Interpreter',interp_tex, 'FontName',fnt, 'FontSize',fs);

    if ~isempty(eta_ex_T)
        legend({'$\eta_h$', '$\eta_{\mathrm{ex}}$'}, ...
               'Interpreter',interp_tex, 'Location','best', ...
               'FontName',fnt, 'FontSize',fs-8, 'Box','on');
    end
    set(gca, 'FontName',fnt, 'FontSize',fs-4, 'TickLabelInterpreter','latex');

    % ---------- SUBPLOT 2: q ----------
    subplot(2,1,2);
    hold on; box on; grid on;
    set(gca, 'GridLineStyle','-', 'GridAlpha',0.15);

    plot(x, full(q_h_T), '-', 'LineWidth',lw, 'Color',clr(1,:));
    if ~isempty(q_ex_T)
        plot(x_ex, q_ex_T, '--', 'LineWidth',lw, 'Color',clr(2,:));
    end

    title(sprintf('$q$ at time %.1f\\,s with $\\Delta t=%.2f$\\,ms, $h=%.2f$, $p=%g$.', ...
          Data.PicTime, Data.dt*1000, 1/femregion.ne, Data.p), ...
          'Interpreter',interp_tex, 'FontName',fnt, 'FontSize',fs);
    xlabel('$x$', 'Interpreter',interp_tex, 'FontName',fnt, 'FontSize',fs);
    ylabel('$q$', 'Interpreter',interp_tex, 'FontName',fnt, 'FontSize',fs);

    if ~isempty(q_ex_T)
        legend({'$q_h$', '$q_{\mathrm{ex}}$'}, ...
               'Interpreter',interp_tex, 'Location','best', ...
               'FontName',fnt, 'FontSize',fs-8, 'Box','on');
    end
    set(gca, 'FontName',fnt, 'FontSize',fs-4, 'TickLabelInterpreter','latex');

    if Data.save_sol_images
        filename = sprintf('%s_nEL_%g_p_%g_dt_%g_t_%g.pdf', ...
                           Data.name, femregion.ne, Data.p, Data.dt, Data.PicTime);
        exportgraphics(fig1, fullfile('Plots', filename), 'ContentType','vector');
    end
end

% % Plot final time
% if Data.visual_graph
%     figure(2); clf;
% 
%     fs = 25;   % SAME font size as all other figures
% 
%     % ---------- SUBPLOT 1: eta ----------
%     subplot(2,1,1);
%     plot(x, full(eta_h_T), 'LineWidth', 1.9); hold on;
%     if ~isempty(eta_ex_T)
%         plot(x, eta_ex_T, '--', 'LineWidth', 1.9);
%     end
%     grid on;
% 
%     title(sprintf('$\\eta$ at time %.1f\\,s with $\\Delta t=%.2f$\\,ms, $h=%.2f$', ...
%           Data.PicTime, Data.dt*1000, 1/femregion.ne), ...
%           'Interpreter','latex', ...
%           'FontName','Times New Roman', ...
%           'FontSize',fs);
% 
%     xlabel('$x$', ...
%            'Interpreter','latex', ...
%            'FontName','Times New Roman', ...
%            'FontSize',fs);
% 
%     ylabel('$\eta$', ...
%            'Interpreter','latex', ...
%            'FontName','Times New Roman', ...
%            'FontSize',fs);
% 
%     if ~isempty(eta_ex_T)
%         legend({'$\eta_h$', '$\eta_{\mathrm{ex}}$'}, ...
%                'Interpreter','latex', ...
%                'Location','best', ...
%                'FontName','Times New Roman', ...
%                'FontSize',fs);
%     end
% 
% 
%     % ---------- SUBPLOT 2: q ----------
%     subplot(2,1,2);
%     plot(x, full(q_h_T), 'LineWidth', 1.9); hold on;
%     if ~isempty(q_ex_T)
%         plot(x, q_ex_T, '--', 'LineWidth', 1.9);
%     end
%     grid on;
% 
%     title(sprintf('$q$ at time %.1f\\,s with $\\Delta t=%.2f$\\,ms, $h=%.2f$', ...
%           Data.PicTime, Data.dt*1000, 1/femregion.ne), ...
%           'Interpreter','latex', ...
%           'FontName','Times New Roman', ...
%           'FontSize',fs);
% 
%     xlabel('$x$', ...
%            'Interpreter','latex', ...
%            'FontName','Times New Roman', ...
%            'FontSize',fs);
% 
%     ylabel('$q$', ...
%            'Interpreter','latex', ...
%            'FontName','Times New Roman', ...
%            'FontSize',fs);
% 
%     if ~isempty(q_ex_T)
%         legend({'$q_h$', '$q_{\mathrm{ex}}$'}, ...
%                'Interpreter','latex', ...
%                'Location','best', ...
%                'FontName','Times New Roman', ...
%                'FontSize',fs);
%     end
% 
% 
%     if Data.save_sol_images
%         filename = sprintf('%s_nEL_%g_p_%g_dt_%g_t_%g.pdf', Data.name, femregion.ne, Data.p, Data.dt, Data.PicTime);
%         exportgraphics(gcf, fullfile('Plots', filename), ...
%                    'ContentType','vector');
%     end
% end

% Save
solutions = struct( ...
    'x', x, ...
    'eta_h', eta_h_T, ...
    'q_h', q_h_T, ...
    'eta_ex', eta_ex_T, ...
    'q_ex', q_ex_T );

fprintf('============================================================\n')
end
